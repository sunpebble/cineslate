    ...
    @ViewBuilder private var centerState: some View {
        ...
        case .loading:
            VStack(spacing: 12) {
                ...
                Text(String(localized: "正在连接 Plex…"))
        ...
        case .failed(let message):
            VStack(spacing: 16) {
                ...
                Button(String(localized: "返回")) { dismiss() }
                    .font(.system(size: 15, weight: .bold))
...
private struct SubtitlePickerSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(String(localized: "字幕"))
    ...
    @ViewBuilder private var content: some View {
        ...
        case .unconfigured:
            infoState(icon: "key.slash",
                      text: String(localized: "前往「设置 → OpenSubtitles」填写 API Key 后，即可在线搜索并加载字幕。"))
        case .searching:
            busyState(String(localized: "正在搜索字幕…"))
        default:
            ScrollView {
                VStack(spacing: 10) {
                    if case .loading = model.subtitleStatus { busyRow(String(localized: "正在加载字幕…")) }
                    ...
                    if model.subtitleOptions.isEmpty, case .empty = model.subtitleStatus {
                        infoState(icon: "text.magnifyingglass", text: String(localized: "没有找到匹配的字幕。"))
                            .padding(.top, 20)
                    } else {
    ...
    private var offRow: some View {
        Button {
            ...
            HStack {
                Text(String(localized: "关闭字幕")).font(.system(size: 15, weight: .semibold)).foregroundStyle(.white)
                Spacer()
