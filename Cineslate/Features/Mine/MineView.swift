...
struct MineView: View {
    ...
    private var titleRow: some View {
        HStack {
            HStack(spacing: 10) {
                Text(String(localized: "我的")).font(.system(size: 32, weight: .black)).kerning(-0.5)
    ...
    private var emptyState: some View {
        VStack(spacing: 10) {
            ...
            Text(String(format: String(localized: "「%@」还是空的"), segment.title))
                .font(.system(size: 15, weight: .semibold))
                .foregroundStyle(RFX.text3)
            Text(String(localized: "去发现页挑一部，加入这里"))
                .font(.system(size: 13))
                .foregroundStyle(RFX.text4)
            Button(String(localized: "去发现")) { router.select(.discover) }
    ...
    private func cta(for list: LibraryList) -> String {
        switch list {
        case .watching: return String(localized: "继续观看")
        case .upcoming: return String(localized: "查看")
        case .watchLater: return String(localized: "查看")
        case .history: return String(localized: "重新观看")
        case .favorite: return String(localized: "查看")
        }
    }
    ...
    private var featuredItem: FeaturedData? {
        ...
        if let fb = model.fallbackFeatured {
            return FeaturedData(
                ...
                subtitle: String(localized: "为你推荐"),
...
private struct LibraryRow: View {
    ...
    var body: some View {
            ...
            HStack(alignment: .top, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    HStack(alignment: .bottom) {
                        Text(item.mediaType == "tv" ? String(localized: "剧集") : String(localized: "电影"))
        ...
        .contextMenu {
            Button(role: .destructive, action: onRemove) {
                Label(String(localized: "从列表移除"), systemImage: "trash")
            }
        }
    }
    ...
    private var metaText: String {
        var parts: [String] = []
        parts.append(item.mediaType == "tv" ? String(localized: "剧集") : String(localized: "电影"))
        if let runtime = item.runtimeMinutes, runtime > 0 { parts.append(String(format: String(localized: "%lld 分钟"), runtime)) }
