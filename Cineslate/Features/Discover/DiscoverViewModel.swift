...
final class DiscoverViewModel: ObservableObject {
    ...
    /// Namespaced by content language so a language switch doesn't render a
    /// stale snapshot from the other locale.
    private var cacheKey: String { "discover-\(TMDBService.contentLanguage)" }
    ...
    func loadIfNeeded() async {
        ...
        if let entry = await DiskCache.shared.load(cacheKey, as: DiscoverSnapshot.self) {
            apply(entry.payload)
            hasLoaded = true
            if !entry.isFresh(ttl: CacheTTL.discover) {
                ...
                await loadArtwork()
                await DiskCache.shared.save(cacheKey, snapshot())
            }
        } else {
            await reload()
        }
    ...
    func reload() async {
        ...
        do {
            ...
            loadError = (error as? LocalizedError)?.errorDescription ?? String(localized: "加载失败，请检查网络或 API Key")
        }
        isLoading = false
    }
